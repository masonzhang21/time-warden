...
export default {
  ...
  async isWatchedSite(site) {
    const sites = await this.getWatchedSites();
  ...
  async addTime(minutes, siteURL, date) {
    ...
    const lastSunday = Date(times.timestamp);
  ...
  async newWeek() {
    ...
    times.timestamp = times.timestamp.setDate(times.timestamp.getDate() + 7);
  ...
  async endTrackerSession(sessionStart, siteURL) {
    ...
    if (!this.isSameDay(sessionStart, sessionEnd)) {
      ...
      this.addTime(minutesOnPreviousDay, sessionStart);
